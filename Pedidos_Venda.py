...
# Função para buscar dados da tabela PCPEDI com cache
...
def fetch_pedidos(data_inicial, data_final):
    ...
    if key not in cache:
        try:
            ...
            response = supabase.table(SUPABASE_TABLE).select("*").gte("DATA", data_inicial_str).lte("DATA", data_final_str).execute()
            data = response.data
...
def main():
    ...
    with st.container(border=True):
        ...
        with col1:
            data_inicial = st.date_input("Data Inicial", date.today(), key="data_inicial")
        with col2:
            data_final = st.date_input("Data Final", date.today(), key="data_final")
        st.divider()
