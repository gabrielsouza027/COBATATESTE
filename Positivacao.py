...
def main():
    ...
    def fetch_data(data_inicial, data_final):
        try:
            response = supabase.table('PCVENDEDOR').select('*').gte('DATAPEDIDO', data_inicial.strftime("%Y-%m-%d")).lte('DATAPEDIDO', data_final.strftime("%Y-%m-%d")).execute()
            if response.data:
    ...
    def process_year_month_summary(df, selected_year, selected_month):
        ...
        required_columns = ['DATAPEDIDO', 'VALOR', 'QUANTIDADE', 'CUSTOPRODUTO', 'CODIGOVENDA', 
                           'CODCLIENTE', 'VENDEDOR', 'CLIENTE', 'FANTASIA', 'RAMO', 'PEDIDO']
        missing_columns = [col for col in required_columns if col not in df.columns]
        if missing_columns:
            st.error(f"Colunas obrigatórias ausentes: {', '.join(missing_columns)}")
            return pd.DataFrame()
        ...
        # Agrupar por VENDEDOR, CODCLIENTE, CLIENTE, FANTASIA, RAMO
    ...
    with detailed_placeholder.container():
        ...
        for idx, report in enumerate(st.session_state.detailed_reports):
            with st.container():
                ...
                gb.configure_column("CLIENTE", header_name="Cliente", width=150)
    ...
    with year_month_placeholder.container():
        ...
        if not df.empty:
            ...
            if not available_months:
                available_months = [date.today().month]
        ...
        for idx, summary_info in enumerate(st.session_state.year_month_summaries):
            with st.container():
                ...
                gb.configure_column("CLIENTE", header_name="Cliente", width=150)
                gb.configure_column("FANTASIA", header_name="Fantasia", width=150)
                gb.configure_column("RAMO", header_name="Ramo", width=120)
